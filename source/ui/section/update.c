...
static void update_check_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_SUCCEEDED(res = http_download_json("https://api.github.com/repos/EMUGamesDevTeam/FBI/releases/latest", &json, 16 * 1024))) {
        if(json_is_object(json)) {
            json_t* name = json_object_get(json, "tag_name");
    ...
    if(hasUpdate) {
        action_url_install("Update FBI to the latest version?", updateURL, fs_get_3dsx_path(), NULL, NULL, NULL, NULL);
    } else {
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "Failed to check for update.");
        } else {
            prompt_display("Success", "No updates available.", COLOR_TEXT, NULL, NULL, NULL);
        }
    }
}
